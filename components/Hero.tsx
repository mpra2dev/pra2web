...
export function Hero() {
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-950 px-4 pb-14 pt-1 sm:px-6 sm:pb-24 sm:pt-2 lg:px-8 lg:pt-2">
      <style>{`
      ...
      <div
        ref={ref}
        className={`relative z-10 mx-auto grid max-w-6xl items-center gap-8 transition-all duration-700 ease-out sm:gap-10 lg:min-h-[calc(100svh-3.75rem)] lg:grid-cols-[1.2fr_0.8fr] lg:gap-14 ${
      ...
      >
        ...
        <div className="order-1 flex flex-col items-center text-center lg:items-start lg:text-left">
          ...
          <h1 className="mt-6 max-w-5xl text-balance text-4xl font-semibold tracking-[-0.06em] text-white sm:mt-8 sm:text-6xl lg:text-7xl lg:leading-[0.94] xl:text-8xl">
            ...
            <span className="mt-2 block text-2xl font-medium tracking-[-0.04em] text-white/75 sm:mt-3 sm:text-4xl lg:text-5xl">
              (o por su dise&ntilde;o).
            </span>
          </h1>

          {/* Supporting copy explains the tension: traffic and budget exist, but conversion does not. */}
          <p className="mt-6 max-w-3xl text-pretty text-base leading-7 text-neutral-300 sm:mt-8 sm:text-xl sm:leading-9">
          ...
          <p className="mt-4 text-xl font-semibold tracking-[-0.04em] text-cyan-200 sm:mt-6 sm:text-3xl">
            Nosotros s&iacute;.
          </p>

          {/* Buttons stay grouped under the message so the next step remains obvious. */}
          <div className="mt-8 flex w-full max-w-md flex-col gap-3 sm:mt-12 sm:gap-4 sm:flex-row lg:max-w-none lg:justify-start">
            <a
              href="#contacto"
              className="inline-flex min-w-[220px] items-center justify-center rounded-full border border-cyan-300/20 bg-white px-6 py-3.5 text-sm font-semibold text-neutral-950 shadow-[0_12px_34px_rgba(255,255,255,0.14)] transition-all duration-200 hover:-translate-y-0.5 hover:bg-cyan-50 hover:shadow-[0_18px_40px_rgba(125,211,252,0.2)] sm:px-7 sm:py-4"
            >
              Habla con nosotros
            </a>
            <a
              href="#proceso"
              className="hidden min-w-[220px] items-center justify-center rounded-full border border-white/12 bg-white/4 px-7 py-4 text-sm font-semibold text-white/90 transition-all duration-200 hover:border-white/20 hover:bg-white/8 hover:text-white sm:inline-flex"
            >
        ...
        <div className="order-2 mx-auto hidden w-full max-w-sm grid-cols-1 gap-2.5 sm:grid sm:grid-cols-3 sm:gap-3 lg:mx-0 lg:max-w-none lg:grid-cols-1 lg:justify-self-end">
          <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3.5 backdrop-blur-sm sm:px-5 sm:py-4">
